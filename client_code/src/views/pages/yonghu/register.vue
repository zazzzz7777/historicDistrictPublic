...
const handleRegister = () => {
  ...
  if (!registerForm.value.yonghuming) {
  ...
  if (
    registerForm.value.maxPasswordWrong &&
    !context?.$toolUtil.isIntNumer(registerForm.value.maxPasswordWrong)
  ) {
    context?.$toolUtil.message(`最大密码输错次数应输入整数`, 'error')
    return false
  }
  if (
    registerForm.value.isLocked &&
    !context?.$toolUtil.isIntNumer(registerForm.value.isLocked)
  ) {
...
.register_view {
  background-image: url('http://localhost:8080/historicDistrict/file/d35e3adb230a47b7bbd4e9b1d3b2bb75.jpg');
  // 标题盒子
  .outTitle_view {
    .outTilte {
    }
...
.register_view {
  min-height: 100vh;
  position: relative;
  background: url(http://localhost:8080/historicDistrict/file/d35e3adb230a47b7bbd4e9b1d3b2bb75.jpg)
...
.register_view .register_form {
  ...
  box-shadow: 5px 5px 0px 0px var(--theme2-color),
    10px 10px 0px 0px var(--theme2-color50),
    15px 15px 0px 0px var(--theme2-color30),
    20px 20px 0px 0px var(--theme2-color20),
    25px 25px 0px 0px var(--theme2-color10);
}
