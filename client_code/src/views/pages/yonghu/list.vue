		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			<div class="search_view">
				...
				<div class="search_box">
					<el-input class="search_inp" v-model="searchQuery.yonghuming" placeholder="用户名"
						clearable>
					</el-input>
				</div>
			</div>
			...
			<div class="data_box">
                <div class="table_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
							...
							<template #default="scope">
								<div v-if="scope.row.touxiang">
									<el-image v-if="scope.row.touxiang.substring(0,4)=='http'" preview-teleported
										...
										:src="scope.row.touxiang.split(',')[0]" style="width:100px;height:100px" @click.stop></el-image>
									<el-image v-else preview-teleported
										:preview-src-list="[$config.url+scope.row.touxiang.split(',')[0]]"
										:src="$config.url+scope.row.touxiang.split(',')[0]" style="width:100px;height:100px" @click.stop>
									</el-image>
								</div>
								<div v-else>无图片</div>
							</template>
						...
						<el-table-column label="性别" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.xingbie}}
							</template>
						</el-table-column>
...
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.yonghuming&&searchQuery.value.yonghuming!=''){
			params.yonghuming = '%' + searchQuery.value.yonghuming + '%'
		}
		context?.$http({
