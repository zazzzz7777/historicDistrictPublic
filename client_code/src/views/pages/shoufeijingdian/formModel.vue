			...
			<div class="info_view">
				<div class="title_view">
					<div class="detail_title">
						<span>{{detail.jiequmingcheng}}</span>
					</div>
				...
				<div class="info_item">
					<div class="info_label">景点名称</div>
					<div  class="info_text" >{{detail.jingdianmingcheng}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					...
					<div  class="info_text" >{{detail.dizhi}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					...
					<div  class="info_text" >{{detail.clickNumber}}</div>
				</div>
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth('shoufeijingdian','购票')" @click="goupiaoxinxionAcross('购票','goupiaoxinxi','是','','','')" type="warning">购票</el-button>
					<el-button class="approval_btn" v-if="btnAuth('shoufeijingdian','审核')" type="warning" @click="approvalClick()">审核</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('shoufeijingdian','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('shoufeijingdian','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
            ...
            title.value = res.data.data.jiequmingcheng
	...
	const approvalClick = (btnType='审核') => {
		...
		if(!btnAuth('shoufeijingdian',btnType)){
	...
	const goupiaoxinxionAcross = async (btnType,table,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
