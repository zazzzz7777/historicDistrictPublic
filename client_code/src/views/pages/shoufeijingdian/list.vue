	...
	<div class="list-page" :style='{}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('shoufeijingdian','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
        ...
        <div class="sort-wrapper">
            <el-button class="item click" @click="sortClick('click_number')" :class="{active:sortType=='click_number'}">
                <el-icon class="icon" v-if="sortType!='click_number'"><DCaret /></el-icon>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.jingdiantupian && item.jingdiantupian.substr(0,4)=='http'" @click.stop="preViewClick(item.jingdiantupian)">
							<el-image class="data_img" :src="item.jingdiantupian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.jingdiantupian.split(',')[0])">
							<el-image class="data_img" :src="item.jingdiantupian?$config.url + item.jingdiantupian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title1">
								<span>{{item.jiequmingcheng}}</span>
							</div>
							<div class="data_title2">
								<span>门票价格：{{item.menpiaojiage}}</span>
							</div>
							<div class="data_title3">
								<span>开放时间：{{item.kaifangshijian}}</span>
							</div>
							<div class="data_title4">
								<span>地址：{{item.dizhi}}</span>
							</div>
						</div>
					</div>
				</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/shoufeijingdianAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.jingdianleixing = categoryList.value[categoryIndex.value]
		}
		...
		if (searchQuery.value.menpiaojiagestart&&searchQuery.value.menpiaojiagestart!='') {
			params.menpiaojiagestart = searchQuery.value.menpiaojiagestart
		}
		if (searchQuery.value.menpiaojiageend&&searchQuery.value.menpiaojiageend!='') {
