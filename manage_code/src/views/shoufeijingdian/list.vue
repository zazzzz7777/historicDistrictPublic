<template>
	<div>
		<div class="center_view">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.menpiaojiagestart" placeholder="最小门票价格"
								clearable>
							</el-input>至
							<el-input class="search_inp" v-model="searchQuery.menpiaojiageend" placeholder="最大门票价格"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<div class="btn_view">
					<el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('shoufeijingdian','新增')">
			...
			<br>
			<div class="dataList"
				 v-if="btnAuth('shoufeijingdian','查看')"
			>
				<div class="item" v-for="(item,index) in list" :key="item.id">
					...
					<div class="title">门票价格：{{item.menpiaojiage}}</div>
					...
					<div class="btns">
						<el-button class="view_btn" type="info" v-if=" btnAuth('shoufeijingdian','查看')" @click="infoClick(item.id)">
							<i class="iconfont icon-sousuo3"></i>
							详情
						</el-button>
						<el-button class="edit_btn" type="primary" @click="editClick(item.id)" v-if=" btnAuth('shoufeijingdian','修改')">
							<i class="iconfont icon-xiugai4"></i>
							修改
						</el-button>
						<el-button class="del_btn" type="danger" @click="delClick(item.id)"  v-if="btnAuth('shoufeijingdian','删除')">
							<i class="iconfont icon-shanchu1"></i>
							删除
						</el-button>
						<el-button class="cross_btn" v-if="btnAuth('shoufeijingdian','购票')" type="success" @click="goupiaoxinxiCrossAddOrUpdateHandler(item,'cross','是','','','')">
							<i class="iconfont icon-dingdan2"></i>
							购票
						</el-button>
                        <el-button class="operate_btn" v-if="item.sfsh!='是' && btnAuth('shoufeijingdian','审核')" @click="approvalClick(item)" type="warning">审核</el-button>
						<el-button class="operate_btn" v-if="btnAuth('shoufeijingdian','查看评论')" type="warning" @click="commentClick(item.id)">
		...
		<goupiaoxinxiFormModel ref="goupiaoxinxiFormModelRef" @formModelChange="formModelChange"></goupiaoxinxiFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.menpiaojiagestart){
			params['menpiaojiagestart'] = searchQuery.value.menpiaojiagestart
		}
		if(searchQuery.value.menpiaojiageend){
			params['menpiaojiageend'] = searchQuery.value.menpiaojiageend
		}
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussshoufeijingdian?refid=' + id)
	}
	import goupiaoxinxiFormModel from '@/views/goupiaoxinxi/formModel'
	const goupiaoxinxiFormModelRef = ref(null)
    const goupiaoxinxiCrossAddOrUpdateHandler = (row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			goupiaoxinxiFormModelRef.value.init(row.id,'cross','购票',row,'shoufeijingdian',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
